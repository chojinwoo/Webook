...
EPUBJS.Reader = function(bookPath, _options) {
	...
	if(this.settings.generatePagination) {
		var pagination = book.generatePagination($viewer.width(), $viewer.height());
	}
...
EPUBJS.reader.ControlsController = function(book) {
	...
	book.on('renderer:locationChanged', function(cfi){
		...
		function page() {
			var nowPage = book.pagination.pageFromCfi(cfi);
			$('#pageMove').attr('placeholder', 'nowPage : '+nowPage);
			time = 300;
			clearInterval(iter);
		}
		var iter = setInterval(page, time);
...
EPUBJS.reader.ReaderController = function(book) {
	...
	$next.on("click", function(e){
		...
		if(book.metadata.direction === "rtl") {
			...
			var lastPage = book.pagination.totalPages;
			var nowPage = book.pagination.pageFromCfi(reader.currentLocationCfi);
			if(lastPage == nowPage) {
				if(confirm('마지막페이지 입니다.\n처음페이지로 이동하시겠습니까?')) {
					book.gotoPage(book.pagination.firstPage);
				}
			} else {
				book.nextPage();
			}
